...
export const employeeOverviewSchema = z.object({
  ...
  company_email: z.string().email("Email Tidak Valid").toLowerCase().nullable().or(z.literal("")),
  ...
  work_employee_status_id: z.number({ error: "Wajib diisi" }).min(1, "Wajib diisi"),
...
export const employeePersonalSchema = z.object({
  ...
  email: z.string().email("Email Tidak Valid").toLowerCase().nullable().or(z.literal("")),
...
export const employeeAttachmentSchema = z.object({
  ...
  kartu_keluarga: z.any().refine((file) => file instanceof File, "Wajib diunggah"),
  ijazah: z.any().refine((file) => file instanceof File, "Wajib diunggah"),
  file_pendukung: z.array(z.any()).min(1, "Wajib diunggah"),
});

export type EmployeeAttachmentFormValues = z.infer<typeof employeeAttachmentSchema>;

export const familySchema = z.object({
  full_name: z.string().min(1, "Wajib diisi").max(255),
  family_relationship_id: z.number({ error: "Wajib diisi" }).min(1, "Wajib diisi"),
...
export const experienceSchema = z.object({
  ...
  start_year: z.string({ error: "Wajib diisi" }).min(4, "Min 4 karakter").max(4),
...
export const licenseSchema = z.object({
  ...
  driver_license_type_id: z.number({ error: "Wajib diisi" }).min(1, "Wajib diisi"),
});
