...
import { useApprovals, useApproveRequest, useRejectRequest, ApprovalCategory } from '@/modules/approval-workflow/actions/hooks/use-approvals';
...
export function ApprovalInboxClient() {
  ...
  const { approvals, isLoading, summary_counts, mutate } = useApprovals(activeTab, {
  ...
  return (
    <div className="space-y-6 w-full pb-10">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as ApprovalCategory)} className="w-full space-y-4">
        <TabsList className="gap-6 w-full ">
          {[
            { id: 'pending', label: 'Inbox', count: summary_counts?.pending },
            { id: 'upcoming', label: 'Mendatang', count: summary_counts?.upcoming },
            { id: 'ongoing', label: 'Berjalan', count: summary_counts?.ongoing },
          ].map((tab) => (
            <TabsTrigger
              key={tab.id}
              value={tab.id}
              className="relative px-0 py-3 text-xs data-[state=active]:bg-primary data-[state=active]:text-white transition-all"
            >
              {tab.label}
              {tab.count !== undefined && tab.count > 0 && (
                <span className="ml-2 px-1.5 py-0.5 bg-primary/10 data-[state=active]:bg-white rounded-full text-[10px] font-bold">
                  {tab.count}
                </span>
              )}
            </TabsTrigger>
        ...
        <div className="outline-none pt-2">
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => <ApprovalCardSkeleton key={i} />)}
            </div>
          ) : approvals.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 bg-muted/5 rounded-xl border border-dashed border-muted/20">
              <HugeiconsIcon icon={CheckmarkCircle02Icon} className="h-10 w-10 text-muted-foreground/20 mb-3" />
              <h3 className="text-sm font-medium text-muted-foreground">Tidak ada data untuk kategori ini.</h3>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
