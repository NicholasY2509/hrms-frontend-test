      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">
            {row.original.warning_at ? format(new Date(row.original.warning_at), 'dd MMM yyyy', { locale: localeId }) : '-'}
          </span>
        </div>
      ),
    },
      ...
      cell: ({ row }) => (
        <div className="flex flex-col gap-1">
          <span className="text-xs text-muted-foreground">
            {row.original.start_date ? format(new Date(row.original.start_date), 'dd MMM yyyy', { locale: localeId }) : '-'}
            {" - "}
            {row.original.end_date ? format(new Date(row.original.end_date), 'dd MMM yyyy', { locale: localeId }) : '-'}
          </span>
        </div>
      ),
    },
