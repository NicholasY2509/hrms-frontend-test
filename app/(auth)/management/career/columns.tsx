  ...
  onView: (item: CareerModel) => void
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        let variant: 'default' | 'warning' | 'info' | 'success' | 'destructive' | 'secondary' = 'secondary';
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-3 py-1">
          <div className="flex flex-col gap-1 min-w-[120px]">
            <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Sebelum</span>
            <div className="flex items-center gap-1.5">
              <HugeiconsIcon icon={Briefcase01Icon} size={12} className="text-muted-foreground" />
              <span className="text-xs font-medium truncate max-w-[150px]">
                {row.original.before_work_position?.name || '-'}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <HugeiconsIcon icon={Building01Icon} size={12} className="text-muted-foreground" />
          ...
          <HugeiconsIcon icon={ArrowRight01Icon} size={14} className="text-muted-foreground/50 shrink-0" />

          <div className="flex flex-col gap-1 min-w-[120px]">
            <span className="text-[10px] uppercase tracking-wider text-primary font-bold">Sesudah</span>
            <div className="flex items-center gap-1.5">
              <HugeiconsIcon icon={Briefcase01Icon} size={12} className="text-primary" />
              <span className="text-xs font-semibold truncate max-w-[150px]">
                {row.original.after_work_position?.name || '-'}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <HugeiconsIcon icon={Building01Icon} size={12} className="text-primary/70" />
              <span className="text-xs text-muted-foreground truncate max-w-[150px]">
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">
            {row.original.career_at ? format(new Date(row.original.career_at), 'dd MMM yyyy', { locale: localeId }) : '-'}
          </span>
        </div>
      ),
    },
