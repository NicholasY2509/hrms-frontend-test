            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="text-2xl font-bold tracking-tight">Detail Pengajuan Lembur</h1>
                            {getStatusBadge(item.status)}
                        </div>
                        <p className="text-muted-foreground text-sm font-medium">ID: {item.id} • No. Dokumen: <span className="text-foreground font-bold">{item.document_no || '-'}</span></p>
                    </div>
                </div>
                {(item.status === 'Approved' || item.status === 'Settled') && (
                    <OvertimeSettleDialog
                        item={item}
                        trigger={
                            <Button disabled={item.status === 'Settled'} className='flex flex-row items-center gap-2'>
                                <HugeiconsIcon icon={CheckmarkCircle01Icon} className="h-4 w-4" /> {item.status === 'Settled' ? 'Telah Disettle' : 'Settle'}
                            </Button>
                        }
                    />
                )}
            ...
            <div className="grid gap-6 md:grid-cols-3">
                ...
                <div className="space-y-6">
                    <ApprovalHistory
                        ...
                        onActionSuccess={() => {
                            mutate();
                            queryClient.invalidateQueries({ queryKey: [OVERTIME_ENDPOINTS.PORTAL.MANAGEMENT.LIST] });
                        }}
                    />
