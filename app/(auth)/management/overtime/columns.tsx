...
export const getOvertimeColumns = (
  onView: (item: Overtime) => void
): ColumnDef<Overtime>[] => [
  {
    accessorKey: "document_no",
    header: "Nomor Pengajuan",
    cell: ({ row }) => (
      <span className="font-medium">{row.original.document_no}</span>
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col">
        ...
        <span className="text-xs text-muted-foreground">
          {format(new Date(row.original.start_time), "HH:mm")} s/d{" "}
          {format(new Date(row.original.finish_time), "HH:mm")}
        </span>
      </div>
    ),
  },
