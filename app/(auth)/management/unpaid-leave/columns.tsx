...
import { ProfileIcon, PencilEdit01Icon, Calendar01Icon, UserIcon } from '@hugeicons/core-free-icons';
        ...
        return (
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5 text-sm">
              ...
              <span className='text-xs'>{format(start, 'dd MMM yyyy', { locale: localeId })}</span>
              {row.original.start_date !== row.original.end_date && (
                <>
                  <span className="text-muted-foreground">-</span>
                  <span className='text-xs'>{format(end, 'dd MMM yyyy', { locale: localeId })}</span>
                </>
              )}
            </div>
            <span className="text-xs text-muted-foreground mt-0.5">{row.original.total_days} Hari</span>
          </div>
        );
      },
    },
